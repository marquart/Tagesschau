...
from RNNTagger import RNNTagger, NNModule #PyRNN.
...
class CRFTagger(NNModule):
    ...
    def __init__(self, num_chars, num_tags, char_emb_size, word_emb_size,
                 ...
                 dropout_rate, beam_size):

        super(CRFTagger, self).__init__()

        # simple LSTMTagger which computes tag scores
        self.base_tagger = RNNTagger(num_chars, num_tags, char_emb_size, word_emb_size,
                                     char_rec_size, word_rec_size, char_rnn_depth,
                                     word_rnn_depth, dropout_rate)
    ...
    def forward(self, fwd_charIDs, bwd_charIDs, word_embs, tags=None):

        annotation_mode = (tags is None)

        scores = self.base_tagger(fwd_charIDs, bwd_charIDs, word_embs)
        
        # extract the tags with the highest scores for each word and their scores
        ...
        if self.training:  # not done during dev evaluation
            ...
            gs_contained = (best_tags == tags.unsqueeze(1)).long().sum(dim=-1)
