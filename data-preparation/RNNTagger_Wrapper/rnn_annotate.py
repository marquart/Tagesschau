...
sys.path.insert(0,'.')
from RNN_Data import Data #PyRNN.
from RNNTagger import RNNTagger #PyRNN.
from CRFTagger import CRFTagger #PyRNN.

def run_tagging(args, data, hyper_params, model):
   '''
   # load parameters
   ...
   model.eval()
   with torch.no_grad():
      #print([x for x in data.sentences(args.path_data)])
      for i, words in enumerate(data.sentences_from_buffer(args.data_buffer)):
